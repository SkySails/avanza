...
function debug(...message) {
  if (process.env.NODE_ENV === 'development' || true) { //! TODO: REMOVE TRUE
  ...
  return new Promise((resolve, reject) => {
    API({
      ...
      try {
        ...
        debug('Received non-JSON data from API.', response.data)
      }
...
class Avanza extends EventEmitter {
  ...
  authenticate(credentials) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        // if (this._socket) {  //! REMOVE SOCKETS
        //   this._socketRestart()
        // }
  ...
  */
  getPortfolioPerformance(period) {
    const path = constants.paths.PORTFOLIO_PERFORMANCE_PATH
    return this.call('GET', `${path}/${period}`)
  }
